...
#' @param html_output Logical, whether the output should be presented in HTML format.
...
SASfromR <- function(sas_code, indata=NULL, outdata=NULL,
  ...
  html_path <- tempfile(pattern="sas_html_", tmpdir=base_dir)
  dir.create(html_path)
  fig_path <- file.path(html_path,"figs")
  dir.create(fig_path)
  on.exit({
    html_file <- file.path(html_path,"sashtml.htm")
    if (html_output & display_output & file.exists(html_file)) {
